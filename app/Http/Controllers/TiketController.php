...
class TiketController extends Controller
{
    ...
    public function tiketList(Request $request)
    {
        ...
        return DataTables::of($data['dat'])
            ->addColumn('action', function($row){
                ...
                $download_btn = '<a download="'.explode(";",$row["attachment"])[0].'" href="'.Storage::url($document_name).'" target="_blank" class="btn btn-link btn-xs" 
    ...
    public function addTiket(Request $request)
    {
        ...
        } else if($roleid == 'RD004' || $roleid == 'RD005' || $roleid == 'RD006') {
            $status = 'IN PROGRESS';
            $statusid = 'SD002';
            $auth = true;
        } else {
        ...
        if ($auth){
            ...
            // $updateCounter = $this->repository->UPDATECOUNTER($last, $counterid);
            /* Send Email */
    ...
    {   
        ...
        if($category == 'CD001 '){//ketika kategori incindent
            if(!empty($mgrid)){
                ...
                /* Get Email Signto */
                $dataEmailSign = DB::connection('pgsql')->table('master_data.m_user')->where('userid', $assign)->get();
                $emailSign = $dataEmailSign[0]->usermail;
                $assignNameSign = $dataEmailSign[0]->username;
            }
            /* Get Email Requestor */
